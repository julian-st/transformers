...
class ModularConverterTransformer(CSTTransformer):
    ...
    def leave_ClassDef(self, original_node, updated_node):
        ...
        for super_class in bases:
            ...
            if len(list_dependencies.items()) == 0:
            ...
            if len(list_dependencies) > 0:
                ...
                raise ValueError(
                    f"We were unable to find any dependencies for {class_name} (based on inheriting from {super_class})"
                    f"   Here are all the global dependencies that we found in your modular file: {list(class_finder.class_dependency_mapping.keys())}."
                    f"   This usually means that the name of `{class_name}` does not match the pattern of `{super_class}`, or that `{super_class}` was"
                    " not imported from its root definition file."
                )
